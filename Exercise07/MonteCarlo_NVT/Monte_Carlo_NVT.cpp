...
{
    ofstream Epot, Pres;
    //Epot.open(k+"/inst_epot.dat",ios::app);
    //Pres.open(k+"/inst_pres.dat",ios::app);
    
    Epot.open("inst_epot.dat",ios::app);
    Pres.open("inst_pres.dat",ios::app);
...
// contribution to energy and virial
    ...
    stima_pot = 4.0*v/(double)npart + vtail;
    Epot << stima_pot << endl;
    stima_pres = (48.0*w/3.0 + ptail*(double)npart)/vol + rho*temp;
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=12;
    ...
    Epot.open(k+"/output.epot.0",ios::app);
    Pres.open(k+"/output.pres.0",ios::app);
    Gofr.open(k+"/output.gofr.0",ios::app);
    Gave.open(k+"/output.gave.0",ios::app);
