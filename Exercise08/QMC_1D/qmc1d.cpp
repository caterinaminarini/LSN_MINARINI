...
double variationalWaveFunction_second(double v)
{
    ...
    double mu = 0.809209;
    double sigma = 0.622038;
    double S1 = (exp(-(v-mu)*(v-mu)/(2*sigma*sigma)))/(sigma*sigma);
    double S2 = (exp(-(v+mu)*(v+mu)/(2*sigma*sigma)))/(sigma*sigma);
    double S3 = (-1. + (v-mu)*(v-mu)/(sigma*sigma));
    double S4 = (-1. + (v+mu)*(v+mu)/(sigma*sigma));
    return S1*S3 + S2*S4;
